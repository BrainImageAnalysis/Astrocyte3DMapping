%   @ henrik.skibbe
function tissuecyte_stitch_3D(ifolder,ofolder,channel,varargin)
%%
...
switch (channel)
    ...
    otherwise
        ...
        slice_sample = imread([ifolder,'/slice10000.',ext]);
        
        meta.pixelres = [1.385689385 ,1.339262803];
        meta.crop = [50,50,50,50];
        
        
        meta = get_TC_json_info(metadata_folder, meta,~strcmpi(ext,'tif'));
        
        
        if exist(mat_name_in,'file')
        ...
        if exist(ofile2,'file') %&& false
            ...
            if strcmpi(ext,'tif')
                [img3D_raw,img3D_vis]=stitch3D(ifolder,'threshold',threshold,'pixelres',pixelres,'progress_parm',progress_parm,'element_size_xy',element_size_xy,'section_postfix',ext);
            else
...
function [img3D_raw,img3D_vis]=stitch3D(ifolder,varargin)
    ...
    switch section_postfix
        case 'mat'
            filenames=get_filenames(ifolder,'png');
        case 'tif'
            fnames=dir([ifolder,'slice*.tif']);
            filenames = {};
...
function filenames=get_filenames(ifolder,ext)
